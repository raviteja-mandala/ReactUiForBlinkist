...
const [input, setInput] = useState( () => {return { bookTitle : '' , bookAuthor : '' , category : ''}});
...
const validateForm = (userInput) => {
    ...
    if(userInput.hasOwnProperty('bookTitle')){
        console.log(userInput.bookTitle.length+'inside error6'+userInput.bookTitle);
        var bookTitleErrors = [];
...
const handleSubmit = (e) => {
   ...
   if( requiredFieldsNotEmpty && Object.keys(errors).length === 0 ){
    ...
    .then( 
        fetch('http://localhost:3000/books', {
        method: 'POST',
       body : JSON.stringify({id: maxBookId , ...input, min : 20, reads: '0.7k', image: 'book_image1.png' }),
      ...
      .then(json => {
          console.log(json);
          setSuccess('Book with title '+input.bookTitle+' is successfully created!');
      ...
      .catch(err => console.log(err))
       ...
       setSuccess(null);
       setErrors({...errors, submitError : 'Form cannot be submitted with errors!.'})
   }
}
  ...
  console.log(errors);  
    ...
    return (
        ...
        <Grid item><TextField
          ...
          onChange={handleChange}
        /></Grid>
        <Grid item><Button color="primary" variant="contained" onClick={handleSubmit} >ADD BOOK</Button></Grid>
        </Grid>
        </Box>
        </form>
    )
